...
The subsequent 2026-09-14 [Plans scheduling change](sms-plan-scheduling.md)
removes the sender's duplicate time-window check for SMS initiated by Plans.
That change is pending deployment; the historical release below retains its
...
**Timezone hotfix deployed:** `3467de1` corrected the mutually exclusive timezone settings in both builders. Campaigns and Plans reached `UPDATE_COMPLETE` at 19:05:15 and 19:05:48 UTC. The three published function packages match the reviewed assets. AWS accepted one temporary campaign from each corrected builder; both remained `Initialized`, echoed the exact contact configuration, and were deleted without StartCampaign or test calls/SMS. See the [hotfix evidence](/home/devaju/projects/_audit-reports/connect-precall-sms-review-2026-09-11/deployment/timezone-hotfix-2026-09-11/DEPLOYMENT.md). Existing failed runs were preserved rather than automatically replayed.
...
Plans also requires an operator-managed addition: [precall-sms-plans-policy.json](../infra/config/precall-sms-plans-policy.json), inline policy `PrecallSmsPlansAdditionalPerms` on `VipAdminApiPlansStack-FunctionRole111A5701-mSfFlCntjbO0`. The original manual release granted campaign Pause/Resume and invocation of the retry Lambda. The subsequent profile-mode adversarial review adds a proposed `UpdateCampaignSchedule` statement to that file; **this additional statement has not been applied**, and the profile release remains blocked as described in [precall-sms-profile-mode.md](precall-sms-profile-mode.md). The execution record and commands below describe the original manual release, not authorization or validation of the new profile release. `EngineeringPermissionBoundary` prohibits the CloudFormation execution role from modifying IAM policies; the first deployment failed on this pre-existing restriction. Commit `08eef0e` keeps all CloudFormation IAM resources identical to their deployed baseline and provisions the original delta through the authorized operator session. Existing inline policies, the boundary and the CloudFormation service role are preserved.
