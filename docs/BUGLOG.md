...
### INC-03 — `_dispatch_ready_campaigns` reportaba `changed=True` sin verificar progreso real — el "while-loop amplification" diagnosticado en la Sesión 8 (2026-05-19) nunca se arregló en su raíz *(Fix 2026-08-14, deploy 2026-08-18)*
...
## 2026-08-18 — Mapeo dinámico de estados + Location Onboarding Guard

### BD-010 — `suggestCampaignFlow` matcheaba "PA" contra la palabra "CAMPAIGN" — Pennsylvania se enrutaba por el flow de Connecticut *(Fix mismo día)*

- **Síntoma:** al arreglar el mapeo dinámico de PA/TX (ver Features), Pennsylvania empezó a resolver un `segmentStates` no vacío por primera vez — y el modal "Enable Campaign" auto-seleccionaba silenciosamente `campaign-CT` como el flow, con el botón "Create and start" habilitado (antes de este fix, PA nunca llegaba a esta rama, así que el bug estaba latente sin ser alcanzable).
- **Causa raíz:** `EnableCampaignModal.tsx`'s `STATE_FLOW_PATTERNS[state] ?? [state]` — el fallback para estados sin entrada explícita buscaba el código de estado desnudo (`"PA"`) como substring en los nombres de los flows. La palabra `"CAMPAIGN"` contiene literalmente `"PA"` (`'CAMPAIGN-CT'.toUpperCase().includes('PA')` → `true`), así que cualquier flow devuelto primero por la API matcheaba.
- **Fix:** fallback cambiado a `[\`campaign-${state}\`]` (el prefijo canónico completo) en vez del código desnudo — alinea el fallback con la convención de nombres ya documentada en el mismo archivo. `STATE_FLOW_PATTERNS`/`suggestCampaignFlow` exportados para test; 3 tests de regresión agregados, uno reproduce exactamente la colisión CAMPAIGN/PA.
- **Encontrado por:** revisión final de rama completa (Opus) del plan de mapeo dinámico, no por ninguna revisión de tarea individual — el bug solo se volvió alcanzable por el propio fix del plan.
- **Archivos:** `frontend/src/components/EnableCampaignModal.tsx`, `frontend/src/components/EnableCampaignModal.test.ts`.

### BD-011 — Lambda `location_onboarding_guard` habría publicado a SNS sin permiso KMS — alarma desplegada como no-op silencioso *(Fix mismo día, nunca desplegado a producción)*

- **Síntoma:** ninguno observado en producción — encontrado en revisión final antes de cualquier deploy.
- **Causa raíz:** el topic `vip-plans-alerts` está cifrado con una KMS CMK propia (SSE). El role del Lambda nuevo (`LocationOnboardingGuardRole`) recibió `grantPublish` sobre el topic pero ningún permiso `kms:Decrypt`/`kms:GenerateDataKey*` — `grantPublish` de CDK no otorga automáticamente acceso KMS para topics cifrados. Cada intento de publicar habría lanzado `KMSAccessDeniedException`, capturada y solo logueada como warning (sin re-lanzar), y sin ninguna alarma de CloudWatch sobre este Lambda tampoco — la función habría quedado desplegada como un no-op completamente silencioso, indetectable.
- **Fix:** agregado `props.dataKey.grantEncryptDecrypt(guardRole)` (mismo patrón que ya usa el role del Lambda principal para publicar al mismo topic). La excepción del publish ahora se re-lanza (loguea ERROR y `raise`) en vez de tragarse, para que la métrica `Errors` del Lambda se dispare; se agregó una alarma CloudWatch nueva para esta función en `create-alarms.sh`.
- **Verificado independientemente:** política real de la KMS key en producción leída directamente (`aws kms get-key-policy`), confirmado que el grant nuevo aparece en el template CDK sintetizado (`LocationOnboardingGuardRoleDefaultPolicy`) con las acciones `kms:Decrypt`/`kms:GenerateDataKey*` sobre la key correcta.
- **Encontrado por:** revisión final de rama completa (Opus) del plan del Location Onboarding Guard.
- **Archivos:** `infra/lib/stacks/api-plans-stack.ts`, `services/api-plans/src/location_onboarding_guard.py`, `infra/scripts/create-alarms.sh`.

### BD-012 — `resolve_campaign_flow_arn` sin protección ante creación concurrente del mismo flow *(Fix mismo día, no desplegado)*

- **Síntoma:** ninguno observado — riesgo identificado en revisión final antes de que el segundo caller (el modal, vía el endpoint nuevo) llegara a producción.
- **Causa raíz:** `builders.resolve_campaign_flow_arn` es check-then-create sin lock. Antes de hoy su único caller era el executor secuencial de Plans; el nuevo endpoint `POST /plans/resolve-campaign-flow` agrega un segundo caller disparado por un humano (abrir el modal "Enable Campaign"), que puede correr en paralelo al executor. Si ambos compiten por el mismo estado nuevo, ambos fallan en encontrar `campaign-<ESTADO>` y ambos intentan crearlo — el que pierde recibe una excepción de nombre duplicado, que el código colapsaba a `None`, y un `None` en la ruta del executor lanza un `ValueError` que mata una corrida de campaña en vivo aunque el flow ya exista.
- **Fix:** en el bloque `except`, antes de devolver `None`, se vuelve a listar los contact flows una vez y se devuelve el ARN del ganador si el nombre canónico ya existe. Test de regresión agregado simulando exactamente la carrera (create falla, el segundo listado encuentra el flow del ganador).
- **Encontrado por:** revisión final de rama completa (Opus) del plan del Location Onboarding Guard.
- **Archivos:** `services/api-plans/src/builders.py`, `services/api-plans/tests/unit/test_builders.py`.

### BD-013 — Reglas huérfanas de EventBridge agotaron el límite de la política del Lambda, atascando un run 23 horas en silencio *(Fix mismo día, remediado en producción)*

- **Síntoma:** el bucket "2nd Attempt NY/LI" (plan `6203a0b5...`) completó su bucket 6 y no arrancó el bucket 7 durante 23 horas, hasta que se forzó manualmente. `status` del run se quedó en "running" todo ese tiempo, sin `error` seteado.
- **Causa raíz (3 capas):**
  1. `_advance_bucket`'s recovery de `ConcurrentWriteError` crea un nuevo rule de EventBridge (`_schedule_tick`) para el reintento, pero nunca persiste su `scheduleName` — el `save_run` versionado ya había fallado, así que no queda ninguna escritura que cargue ese nombre. El rule (y su permiso en la política del Lambda) queda huérfano para siempre.
  2. `_cleanup_orphan_plan_permissions` solo borra un permiso si su rule **ya no existe** — no hace nada si el rule sigue vivo (justo lo que produce #1), así que la "limpieza proactiva" corría en cada tick sin liberar nunca espacio real.
  3. Al acumularse huérfanos, la política del Lambda (límite duro de AWS: 20480 bytes) se llenó — cualquier `add_permission` legítimo (de cualquier plan) lanzó `PolicyLengthExceededException`, sin capturar, crasheando el tick en silencio.
- **Encontrado por:** el usuario reportó campañas en "warming" tardando mucho + requiriendo start manual; investigación de logs (CloudWatch Insights) + estado real en DynamoDB + `list-rules`/`get-policy` de EventBridge/Lambda.
- **Remediación en producción:** 27 rules huérfanos (de 3 planes distintos) + sus permisos borrados. Política del Lambda pasó de 19803/20480 bytes (677 de margen) a 10488/20480 (9992 de margen).
- **Fix de código:** `store.record_bucket_schedule_name()` persiste el nombre del schedule saltándose el lock de versión, para que el rule de reintento sea rastreable y se pueda borrar normalmente al completar el bucket. `handler.py`: `tick_unhandled_error` ahora manda alerta SNS (antes desaparecía sin rastro — la invocación devolvía `{"ok": False}` sin re-lanzar, así que ni siquiera incrementaba la métrica `Errors`).
- **No arreglado en este pase (documentado, no absorbido silenciosamente):** los IDs truncados a 8 caracteres en el naming de los rules (`vip-plan-<planId[:8]>-run-<runId[:8]>-b<bucket>`) hacen inseguro distinguir "huérfano" de "activo" solo por el SID — el punto ciego de `_cleanup_orphan_plan_permissions` para rules huérfanos que siguen vivos queda sin resolver (el fix real es #1: dejar de crearlos). Tampoco se amplió la alarma `CampaignDispatchStalled` — solo dispara cuando una campaña **existente** revierte a "queued"; no puede detectar un tick que crashea antes de que exista ninguna campaña, que es justo lo que pasó aquí.
- **Archivos:** `services/api-plans/src/store.py`, `services/api-plans/src/executor.py`, `services/api-plans/src/handler.py`, `services/api-plans/tests/unit/test_store.py`, `services/api-plans/tests/unit/test_executor_v2.py`.

---

## 2026-08-27 — Estancamiento de branded campaigns corriendo en paralelo

### BD-014 — `force_start_campaign` no resetea `reconcileRetries` — reintentos manuales agotan el mismo presupuesto que los automáticos *(Fix mismo día, desplegado en producción)*

- **Síntoma:** la campaña CT del plan `f37b0ff2...` quedó `cancelled` con `exitReason=skipped_empty` tras un force-start manual que canceló en 86ms sin reintentar de verdad.
- **Causa raíz:** el reset de Phase 1 en `force_start_campaign` limpia `status`/`startedAt`/`connectCampaignId`/`segmentName`/`segmentArn`/`exitReason`/`errorDetail`/`completedAt`/`brandedCampaignId`/`queueArn`, pero nunca `reconcileRetries`. Los reintentos manuales heredan el contador de intentos previos (automáticos o manuales) en vez de arrancar con presupuesto fresco. Una vez que el contador llega a `reconcile_retry_limit` (default 5), cualquier force-start posterior cae directo a `skipped_empty` sin volver a chequear si el segmento ya tiene leads.
- **Fix:** agregado `cs["reconcileRetries"] = 0` al bloque de reset de Phase 1.
- **Nota:** en el caso real de CT, el segmento seguía genuinamente vacío (confirmado en Redis) — el fix da presupuesto fresco a futuros force-starts, no garantiza que haya leads.
- **Archivo:** `services/api-plans/src/executor.py:force_start_campaign` (Phase 1), test en `services/api-plans/tests/unit/test_executor_v2.py`.

### Investigado, no es un bug — divergencia `_dispatch_cross_bucket_ready` vs `_dispatch_ready_campaigns` en qué cuenta como "padre terminado"

- **Lo que se vio:** TX dependía de CT; CT terminó `cancelled` (no `completed`). El gate permisivo del tick normal (`_dispatch_ready_campaigns`, acepta cualquier estado en `_CAMPAIGN_TERMINAL_STATUSES`) liberó a TX igual; el gate del arranque anticipado cross-bucket (`_dispatch_cross_bucket_ready`, solo acepta `completed` o `exitReason=skipped`) no lo habría liberado.
- **Conclusión tras leer el código:** es asimetría intencional, no bug. `_dispatch_ready_campaigns` implementa a propósito "no cascade-cancel" (documentado en el docstring del módulo) — cualquier desenlace del padre desbloquea al hijo, que se las arregla solo. `_dispatch_cross_bucket_ready` es más conservador porque su acción es más grande: activa todo un bucket futuro por adelantado (nuevo schedule, `bucket.status="running"` antes de su turno). Solo se adelanta con un padre que terminó limpio; si el padre falló/canceló, simplemente no se adelanta — el bucket arranca igual, normalmente, cuando le toque el turno vía la ruta permisiva. No hay deadlock ni trabajo perdido, solo timing.
- **No se tocó código** — comentario aclaratorio agregado en `_dispatch_cross_bucket_ready` para que nadie reabra esta pregunta como bug.

### BD-015 — `_start_one_campaign`'s empty-segment retry path nunca migró a `_slog` (S14-A) *(Fix mismo día, desplegado en producción)*

- **Síntoma:** cero resultados al buscar `empty_segment`/`skipped_empty`/nombre de campaña en CloudWatch Logs para diagnosticar por qué CT se canceló — a pesar de que el evento sí ocurrió. Obligó a reconstruir la secuencia leyendo directamente el estado en DynamoDB en vez de logs.
- **Causa raíz:** S14-A (2026-05-28) migró la mayoría de `executor.py` de `logger` stdlib a `_slog` porque el root logger de Lambda descarta silenciosamente los primeros. La función hermana `_prestart_next_bucket` sí se migró para esta misma lógica de reintento; `_start_one_campaign` (el path real que ejecutó hoy) se quedó atrás — 5 call-sites seguían en `logger.warning`/`logger.info`.
- **Fix:** migrados los 5 call-sites del bloque de reintento de segmento (`_RedisRebuildingError`, `_EmptySegmentError` retry/cancel, excepción genérica, reset final por Redis-not-ready) a `_slog.warn`/`_slog.info`, mismos nombres de evento que ya usa `_prestart_next_bucket` para el equivalente (`start_one_campaign_redis_rebuilding`, `start_one_campaign_empty_segment_retry`, `start_one_campaign_empty_segment_cancelled`, `start_one_campaign_segment_attempt_failed`, `start_one_campaign_redis_not_ready_reset`).
- **Encontrado por:** investigación en vivo del segmento vacío de CT (agente Opus, 2026-08-27).
- **Archivo:** `services/api-plans/src/executor.py:_start_one_campaign`, test en `services/api-plans/tests/unit/test_executor_v2.py`.

### BD-016 — Branded campaigns ignoran `run_duration_minutes` por completo *(Fix mismo día, desplegado en producción)*

- **Síntoma:** una branded campaign de Plan 1.1 configurada con `run_duration_minutes=45` corrió 107 minutos sin cortarse sola — Sebastian tuvo que forzarla a terminar manualmente, abandonando 994 de 1085 contactos sembrados como `EXPIRED`.
- **Causa raíz:** el corte por duración dentro de `tick` vive únicamente en la rama telephony (`if cs["status"] == "running" and cs.get("connectCampaignId"):`) — branded nunca tiene `connectCampaignId` (usa `brandedCampaignId`), así que ese chequeo nunca se ejecutaba para branded. La rama `elif cs.get("brandedCampaignId")...` solo verificaba si la cola de `VipProgressiveCampaignQueue` estaba drenada (`count == 0`), sin ningún chequeo de tiempo transcurrido.
- **Fix:** agregado el mismo chequeo de duración (`_dur`/`_elapsed` desde `run_duration_minutes`, force-stop si `elapsed > dur + 2`) a la rama branded — al dispararse, marca `status="expired"`/`exitReason="expired"` (mismo patrón que `_expire_bucket` ya usa), llama `_write_branded_run_summary` + `_stop_branded_campaign` + emite `BrandedCampaignExpired`. También se actualizó `_write_branded_run_summary` para clasificar `exitReason="expired"` como `ABORTED` (antes caía al `else: ERROR`, categoría incorrecta).
- **Nota:** este fix NO evita que se abandonen contactos pendientes al cortar por duración — eso es el comportamiento esperado/deseado (igual que en telephony). Solo hace que el corte sea automático en vez de requerir intervención manual.
- **Archivo:** `services/api-plans/src/executor.py` (rama branded de `tick`, `_write_branded_run_summary`), test en `services/api-plans/tests/unit/test_executor_v2.py` (también corregido un bug de datetime naive vs aware en el helper `TestTickBrandedPoll._run_with_running_branded`, expuesto al agregar estos tests).

### BD-017 — Nueva métrica: campaña branded activa pero sin progreso de dialing (desplegada)

- **Contexto:** ni `StuckRun` (umbral 4h, nivel de run completo) ni `NoActiveCampaign` (solo mira `status`, no throughput real) detectaban el escenario de hoy — Plan 1.2/2.2 con `status="running"` por 45+ min, agentes disponibles, prácticamente sin avanzar.
- **Fix:** `metrics_collector_handler.py` (corre cada 1 min) ahora compara, por campaña, `contactsPlaced` contra el snapshot de hace 10 min (`VipBrandedCampaignMetrics`). Si no aumentó Y hay agentes disponibles ahora mismo, emite `BrandedCampaignStalled` a `VipBrandedMonitor` (con dimensión `CampaignId`+`PlanId`, y agregado sin dimensiones para una alarma única "cualquier campaña estancada").
- **Bloqueo de deploy encontrado y resuelto:** el role del collector solo tenía `grantWriteData` en `VipBrandedCampaignMetrics` (nunca necesitó leer antes). Agregar `grantReadData`/`grantReadWriteData` vía CDK falló — `cfn-exec-role` no tiene `iam:PutRolePolicy` sobre este role (mismo `EngineeringPermissionBoundary` de siempre) — y el intento de rollback automático de CloudFormation TAMBIÉN falló por el mismo motivo, dejando el stack en `UPDATE_ROLLBACK_FAILED`. Resuelto con `aws cloudformation continue-update-rollback --resources-to-skip <policy>`. El permiso de lectura (`dynamodb:Query` sobre `VipBrandedCampaignMetrics`) se otorgó vía CLI en una policy inline separada (`BrandedMetricsHistoryRead`), y se revirtió el `grantReadData`/`grantReadWriteData` del código CDK para que futuros deploys de este stack no repitan el mismo fallo.
- **Pendiente para Sebastian:** crear la alarma CloudWatch sobre `BrandedCampaignStalled` (namespace `VipBrandedMonitor`) — el código ya emite la métrica, falta la alarma que dispare notificación.
- **Archivo:** `services/api-metrics/src/metrics_collector_handler.py`, `infra/lib/stacks/api-metrics-stack.ts`, test en `services/api-metrics/tests/unit/test_metrics_collector_handler.py` (archivo nuevo — el handler no tenía tests).

### BD-018 — Múltiples campañas branded en la misma cola: la más vieja se comía todos los eventos (desplegado)

- **Síntoma:** en producción, `priority` es siempre `0` para todas las campañas branded — así que `_get_active_campaigns` (ordenado por `priority ASC, createdAt ASC`) degeneraba a FIFO puro por fecha de creación. La campaña más vieja en una cola compartida ganaba el 100% de los eventos de "agente disponible" mientras tuviera un solo contacto pendiente; la más nueva no recibía nada hasta que la primera se vaciaba por completo. El sweep de BD-019 tenía el mismo problema en miniatura: drenaba una campaña por completo (hasta 25 despachos) antes de intentar la siguiente. *(Corrección: esta entrada citaba "BD-014" — error de referencia, BD-014 es un bug no relacionado en `force_start_campaign`; el sweep vive en BD-019.)*
- **Fix (2 archivos):**
  1. `handler_consumer.py`: `_get_active_campaigns` ahora ordena por `priority ASC, lastDispatchedAt ASC` (con `createdAt` como fallback si nunca se despachó) — la campaña que lleva más tiempo esperando gana, no la más vieja por creación. Nueva función `_record_dispatch()` escribe `lastDispatchedAt` en la campaña ganadora tras cada despacho exitoso (best-effort, no bloquea el despacho si falla).
  2. `handler_kickstart.py`: `_process_sweep` pasó de "drenar campaña A por completo, luego B" a interleaving real — un intento por campaña por ronda, rotando hasta que ninguna pueda despachar más.
- **Permiso nuevo requerido:** el role del consumer solo tenía lectura sobre `VipActiveBrandedCampaigns` — se otorgó `dynamodb:UpdateItem` vía policy inline separada (`BrandedLastDispatchedWrite`), mismo patrón que BD-017 (este role es `mutable:false` en CDK, 100% gestionado por CLI, no se intentó vía CDK en absoluto).
- **Archivos:** `services/api-progressive-dialer/src/handler_consumer.py`, `services/api-progressive-dialer/src/handler_kickstart.py`, tests en `test_handler_consumer.py` y `test_handler_kickstart.py`.

### BD-019 — Sweep timer-driven (backstop del "no re-arm") — entrada faltante, agregada retroactivamente

- **Nota:** esta entrada documenta un cambio que se implementó y desplegó el 2026-08-27 pero nunca tuvo su propia entrada en este log — se referenciaba por error como "BD-014" en BD-018 (ya corregido arriba).
- **Síntoma:** el dispatcher de branded es 100% event-driven (evento Kinesis de "agente pasó a Available"). Una campaña con mayoría de llamadas a voicemail nunca hace que un agente cambie de estado de verdad, así que ningún evento vuelve a disparar dispatch — la campaña se queda esperando indefinidamente aunque haya agentes libres y contactos `PENDING`. Esto fue la causa raíz del estancamiento de Plan 1.2/2.2 la mañana del 2026-08-27.
- **Fix:** nuevo entry point en `handler_kickstart.py` — `lambda_handler` rutea eventos de EventBridge (`event.source == "aws.events"`) a `_process_sweep()`, que revisa cada campaña branded activa y despacha a cualquier agente realmente disponible ahora mismo, sin depender de que llegue un evento. Cap de 25 despachos por campaña por tick (`_MAX_SWEEP_DISPATCHES_PER_CAMPAIGN`) para no consumir toda la invocación con un backlog enorme.
- **Infra:** regla EventBridge `vip-progressive-dialer-kickstart-sweep` (`rate(2 minutes)`) apuntando al mismo Lambda kickstart existente — creada vía CLI (no CDK: el role del kickstart es `mutable:false`, 100% gestionado por CLI desde su creación original).
- **Archivo:** `services/api-progressive-dialer/src/handler_kickstart.py`, `infra/lib/stacks/api-progressive-dialer-stack.ts` (importa la regla por ARN, no la crea), tests en `test_handler_kickstart.py`.

### BD-020 — `reconcileRetries` sticky enmascaraba campañas genuinamente atascadas en el detector de "esperas legítimas"

- **Nota:** el código base de este fix (`_bucket_has_only_legitimate_waits`, shortcut `if cs.get("reconcileRetries"): continue`) no fue escrito en la sesión de hoy — ya estaba sin commitear en el repo de una sesión anterior (root-caused 2026-08-27 según su propio docstring, para falsos positivos de `vip-plans-no-active-campaign-sustained` en el plan `1a29f025`). Se documenta aquí porque el bug de fondo se encontró y arregló hoy, y porque nunca tuvo su propia entrada en este log.
- **Síntoma:** `reconcileRetries` se usa como señal de "esta campaña está en medio de un reintento de segmento vacío, no está atascada de verdad" — pero el campo es *sticky*: solo se resetea a 0 dentro del propio ciclo de reintento (`_start_one_campaign`) y en `force_start_campaign` (BD-014). Ninguna otra ruta que revierte una campaña a `queued`/`error` por motivos NO relacionados con ese ciclo lo limpiaba — así que un valor viejo de un ciclo de reintento YA TERMINADO (con éxito o agotado) podía sobrevivir y hacer que el detector tratara una campaña genuinamente crasheada/atascada como "esperando legítimamente", enmascarando el mismo tipo de incidente que BD-013 buscaba detectar.
- **Fix:** se agregó `cs["reconcileRetries"] = 0` en los 3 sitios adicionales que revierten a `queued`/`error` por razones ajenas al ciclo de reintento: `_activate_warming_bucket` (recuperación de pre-warm fallido), `_reset_cascade_cancelled_children` (reset tras force-start del padre), y las 3 ramas de recovery de Phase 1 en `_dispatch_ready_campaigns` (poll de Connect exitoso con estado terminal, excepción al consultar Connect, claim `creating` stale >300s). Docstring de `_bucket_has_only_legitimate_waits` actualizado para reflejar la garantía real (antes decía "su presencia es la señal", lo cual era objetivamente falso sin estos resets).
- **Encontrado por:** revisión adversarial de código (crítico/defensor/juez, 2026-08-27) sobre los deploys del día.
- **Archivo:** `services/api-plans/src/executor.py` (`_activate_warming_bucket`, `_reset_cascade_cancelled_children`, `_dispatch_ready_campaigns`, `_bucket_has_only_legitimate_waits`), tests nuevos en `services/api-plans/tests/unit/test_executor_v2.py`.

### BD-021 — Revisión adversarial de los deploys del 2026-08-27: 7 fixes adicionales (desplegados)

Tras cerrar BD-014 a BD-020, se corrió una revisión adversarial (agente crítico busca fallas → agente defensor refuta o concede → agente juez neutral falla el veredicto leyendo el código él mismo) sobre TODO lo desplegado ese día. Resultado: 10 hallazgos, los 10 confirmados, 0 refutados. BD-020 (arriba) fue uno; el resto de fixes de código (no solo documentación) fueron:

1. **`_process_sweep` sin aislamiento de fallos entre campañas** — una excepción en una campaña (throttling de Connect, item malformado) abortaba el sweep completo para TODAS las demás campañas de ese tick. Fix: try/except por campaña dentro del loop de round-robin, excluye la campaña que falló del resto del tick (no del sweep completo), emite `SweepCampaignDispatchFailed`. También se envolvió la llamada a `_process_sweep()` en `lambda_handler` — un fallo antes del loop (ej. el scan inicial) ya no propaga fuera de la invocación de EventBridge, emite `SweepTickFailed`. Archivo: `handler_kickstart.py`.
2. **Race de lectura-tras-escritura en la fairness fix (BD-018)** — `_get_active_campaigns` lee de una GSI (siempre eventual-consistency en DynamoDB) inmediatamente después de que `_record_dispatch` escribió en la tabla base, dentro de la misma invocación procesando varios records de Kinesis secuencialmente — un record posterior en el mismo batch podía ver el valor viejo y re-elegir la campaña que ya ganó. Fix: overlay en memoria (`_recent_dispatches`, por contenedor Lambda cálido) que se mezcla con el valor leído de la GSI al ordenar, tomando el más reciente de los dos. Archivo: `handler_consumer.py`.
3. **Fallo silencioso al escribir `lastDispatchedAt`** — solo un `logger.warning`, sin métrica; si se pierde el permiso IAM, la fairness fix se degrada a FIFO puro sin ninguna alarma. Fix: `_emit_metric("LastDispatchedAtWriteFailed")` en el except. Archivo: `handler_consumer.py`.
4. **BD-016 evaluaba la duración ANTES de comprobar si la cola ya se había drenado** — una campaña branded que terminó bien en el mismo tick en que se superó la duración configurada se marcaba `expired`/`ABORTED` en vez de `completed`/`queue_drained`. Fix: reordenado para pollear `_count_branded_queue` primero (igual que ya hacía la rama telephony: poll primero, evalúa duración después) — el chequeo de duración solo se alcanza si `count > 0`. Archivo: `executor.py` (rama branded de `tick`).
5. **BD-017 comparaba contra un snapshot sin límite de antigüedad** — `brandedCampaignId` es determinístico por (planId, runId, bucket_index, campaign_index) y sobrevive a un stop/force-restart dentro del mismo run; el primer ciclo post-restart podía comparar contra un snapshot de horas atrás y disparar un falso `BrandedCampaignStalled` justo después de un restart legítimo. Fix: si el snapshot encontrado tiene más de `2×_STALL_LOOKBACK_MINUTES` de antigüedad, se trata como "sin historial suficiente" (no se compara). Archivo: `metrics_collector_handler.py`.
6. **`_count_outcomes` devolvía `(0,0,0,0,0)` fabricado ante cualquier error de DynamoDB** — indistinguible de progreso real cero; ese 0 fabricado alimentaba directamente `_check_and_emit_stall` (falsa alarma de estancamiento) y se persistía como snapshot real en `VipBrandedCampaignMetrics` (contaminando también la comparación del ciclo siguiente). Fix: `_count_outcomes` devuelve `None` en error; `lambda_handler` salta el ciclo completo para esa campaña (sin stall-check, sin `put_item`) en vez de tratar `None` como cero. Archivo: `metrics_collector_handler.py`.
7. **El propio chequeo de estancamiento (BD-017) no se monitoreaba a sí mismo** — si pierde el permiso de lectura, falla en silencio (solo warning), justo el tipo de falla silenciosa que fue creado para detectar. Fix: emite `BrandedStallCheckError` en el except de la query. Archivo: `metrics_collector_handler.py`.

- **Metodología:** workflow con 3 críticos (uno por componente) → 3 defensores → 3 jueces neutrales, cada uno leyendo el código real (no solo confiando en el bando anterior) antes de emitir veredicto.
- **Archivos:** ver cada punto arriba; tests nuevos en `test_handler_kickstart.py`, `test_handler_consumer.py`, `test_executor_v2.py`, `test_metrics_collector_handler.py`.

### BD-022 — Segunda ronda de revisión adversarial sobre los fixes de BD-021: 10 hallazgos más (desplegados)

Se corrió una SEGUNDA ronda del mismo proceso (crítico → defensor → juez), esta vez apuntando específicamente al código nuevo de BD-021, no a los bugs originales. Resultado: 10 hallazgos, los 10 confirmados. **Nota de metodología:** 2 de los 6 agentes defensores tuvieron un fallo de conexión de infraestructura (no relacionado al contenido) — para `progressive-dialer` y `api-metrics-and-infra` el juez verificó de forma independiente sin ver una defensa real; para `api-plans-executor` sí se completó el ciclo completo. Los 10 hallazgos se verificaron de todas formas por lectura directa del código antes de aceptarlos.

1. **`_process_sweep`'s try/except por campaña no protegía contra una excepción DESPUÉS de adquirir el `AgentLock`** (`dequeue()`/`push()`/`send_message()` en `_try_dispatch` no tenían su propio try/except) — el lock quedaba huérfano (leaked) por `_LOCK_TTL_SECONDS` completos, y ahora esa fuga es invisible (antes crasheaba todo el tick de forma ruidosa; el fix de BD-021 la convirtió en un log+métrica silencioso). Fix: `_try_dispatch` envuelve todo su cuerpo post-adquisición en try/except que libera el lock antes de re-lanzar. Archivo: `handler_kickstart.py`.
2. **Test de la fairness fix (BD-018) era no-determinista** — usaba el reloj real (`datetime.now()`) comparado contra un string hardcodeado, solo pasaba porque el reloj del sandbox está en 2026-08-27. Fix: reloj controlado vía mock. Archivo: `test_handler_consumer.py`.
3. **`_recent_dispatches` (overlay en memoria de BD-021) podía resucitar un timestamp stale de ANTES de un stop/restart** — `brandedCampaignId` es determinístico y se reutiliza igual tras un restart dentro del mismo run; el cache local no sabía distinguir "generación anterior" de "generación actual". Fix: se descarta el valor local si es más viejo que el `createdAt` del registro actual. Archivo: `handler_consumer.py`.
4. **Los 2 tests nuevos de aislamiento de fallos del sweep no verificaban el nombre de la métrica**, solo que "se llamó alguna métrica" — un typo o un borrado accidental del nombre real no se habría detectado. Fix: aserciones sobre `MetricName` exacto. Archivo: `test_handler_kickstart.py`.
5. **`_start_one_campaign`'s manejo de `ThrottlingException`/`ServiceQuotaExceededException`** revierte a `queued` sin resetear `reconcileRetries` — un 4to sitio que BD-020 no cubrió. Fix: mismo patrón, `cs["reconcileRetries"] = 0`. Archivo: `executor.py`.
6. **La limpieza de hermanos "warming" en `force_start_campaign`** revierte siblings a `queued` sin resetear `reconcileRetries` — un 5to sitio no cubierto. Fix: agregado al mismo `dict` de reset. Archivo: `executor.py`.
7. **El docstring de `_bucket_has_only_legitimate_waits`** subcontaba los sitios de reset (decía "ambas ramas" cuando son 3 statements) y, tras los hallazgos 5-6, ya no era exhaustivo. Fix: docstring corregido, con nota explícita de que la lista es best-effort, no garantía. Archivo: `executor.py`.
8. **El límite de antigüedad de BD-021 (2× lookback = 20 min) dejaba una ventana de falso positivo entre 10 y 20 min** — la query ya exige que el snapshot encontrado tenga ≥10 min, así que el límite real debía ser mucho más ajustado. Fix: `_STALL_LOOKBACK_MINUTES + 2` (solo margen para jitter del colector) en vez de `× 2`. Archivo: `metrics_collector_handler.py`.
9. **El skip por `_count_outcomes() is None` no emitía métrica**, solo log — mismo punto ciego que BD-021 #7 cerró para `_check_and_emit_stall`, un nivel más arriba. Fix: emite `BrandedOutcomesQueryFailed` (con dimensiones CampaignId/PlanId). Archivo: `metrics_collector_handler.py`.
10. **`BrandedStallCheckError` (BD-021 #7) se emitía sin `Dimensions`**, a diferencia de `BrandedCampaignStalled` — imposible identificar la campaña afectada desde CloudWatch sin ir a logs. Fix: agregadas dimensiones CampaignId/PlanId. Archivo: `metrics_collector_handler.py`.

- **Archivos:** `services/api-progressive-dialer/src/handler_kickstart.py`, `services/api-progressive-dialer/src/handler_consumer.py`, `services/api-plans/src/executor.py`, `services/api-metrics/src/metrics_collector_handler.py`, tests correspondientes en cada servicio.

### BD-023 — Nueva alerta: colisión de cola entre native y branded (desplegado)

- **Contexto:** branded (dispatcher Lambda propio) y native (motor de Connect Campaigns V2) no tienen NINGUNA coordinación entre sí cuando comparten la misma cola de Connect — a diferencia de native+native (Connect gestiona su propio `bandwidthAllocation`) o branded+branded (ya resuelto en BD-018). Decisión explícita de Sebastian: solo alerta, sin bloquear ni cambiar el comportamiento de arranque de ninguna campaña.
- **Diseño — unidireccional por decisión explícita:** al arrancar un campaign **native**, se consulta `VipActiveBrandedCampaigns` por `pk = QUEUE#<queueArn>` (query barata sobre la partition key de la tabla, no requiere GSI) para ver si ya hay un branded activo en esa misma cola. La dirección inversa (branded arrancando mientras ya hay un native activo) NO está cubierta: `campaignConfig.queueId` de native vive en el snapshot del plan/bucket, no en ningún índice consultable por cola, y `VipAdminPlans` no tiene GSI — cubrirla exigiría un scan completo de la tabla en cada arranque de campaña (costo/latencia creciente) o una tabla de tracking nueva tipo `VipActiveNativeCampaigns` con su propio ciclo de vida (superficie de bugs comparable a la de branded). Sebastian eligió explícitamente la cobertura de una sola dirección para esta iteración.
- **Fix:** nueva función `_check_native_queue_collision` en `executor.py`, llamada al entrar a la rama native de `_start_one_campaign` (cubre tanto el arranque fresco como el reinicio de una campaña pre-warmed). Si encuentra un branded activo en la misma cola, emite `_notify_sns` (mismo patrón fire-and-forget que el resto del archivo) y la métrica `BrandedNativeQueueCollision` (namespace `VIPPlans`, con dimensión `QueueId` y agregado sin dimensiones). Nunca bloquea ni altera el arranque — un error en la query se loguea y se ignora, la campaña native sigue su curso normal.
- **Pendiente para Sebastian:** crear la alarma CloudWatch sobre `BrandedNativeQueueCollision` (namespace `VIPPlans`) si se quiere notificación activa — el código ya emite la métrica.
- **Archivo:** `services/api-plans/src/executor.py` (`_check_native_queue_collision`, `_emit_queue_collision_metric`, call site en `_start_one_campaign`), tests nuevos en `services/api-plans/tests/unit/test_executor_v2.py`.
